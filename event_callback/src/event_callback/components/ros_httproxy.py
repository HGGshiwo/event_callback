...
import rospy, json, traceback


class ROSHTTProxyComponent(BaseComponent):
    ...
    def _register_ros_services(self) -> None:
        ...
        # 获取当前组件绑定的所有路由（route）回调
        route_cbs = self.manager_instance.get_callbacks(self._component_name, "route")
        for func in route_cbs:
            # 获取当前路由回调的配置参数（path/method）
            kwargs = self.manager_instance.get_callback_kwargs(
                self._component_name, "route", func
            )
            path, method = kwargs["path"], kwargs["method"]
