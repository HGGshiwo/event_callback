...
class http_proxy(BaseComponentHelper):
    ...
    def post(cls, url: str):
        def register_cb(self: HTTP_ProxyComponent, callback: Callable):
            self._register_ros_services(callback, url, "POST")

        return R._create_comp_decorator(cls.target, register_cb)

    @classmethod
    def get(cls, url: str):
        def register_cb(self: HTTP_ProxyComponent, callback: Callable):
            self._register_ros_services(callback, url, "GET")
